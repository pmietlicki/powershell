...
# Timer pour gestion du runspace async
$timer = New-Object System.Windows.Forms.Timer
$timer.Interval = 1000

$timer.Add_Tick({
    if ($global:PowerShellAsync -and $global:PowerShellAsync.EndInvoke($global:AsyncResult)) {
        $timer.Stop()
        try {
            $results = $global:PowerShellAsync.EndInvoke($global:AsyncResult)
            $global:PowerShellAsync.Dispose()
            $global:Runspace.Close()

            $form.Invoke([System.Action]{
                if ($results -is [string]) {
                    if ($results.StartsWith("ERROR:")) {
                        Write-LogToBox "Erreur : $($results.Substring(6))"
                    }
                    else {
                        Write-LogToBox "Le script a renvoyé un texte inattendu : $results"
                        $dataGridView.Rows.Add("Info", $results)
                    }
                }
                elseif ($results -is [System.Collections.IEnumerable]) {
                    if ($results.Count -gt 0) {
                        Write-LogToBox "$($results.Count) événements trouvés."

                        # Agrégation des postes et des IP uniques
                        $uniqueWS = ($results | Where-Object { $_.PSObject.Properties.Match('WorkstationName') } | Select-Object -ExpandProperty WorkstationName -Unique | Where-Object { $_ -and $_.Trim() -ne "" -and $_.Trim() -ne "-" }) -join ", "
                        $uniqueIPs = ($results | Where-Object { $_.PSObject.Properties.Match('IPAddress') } | Select-Object -ExpandProperty IPAddress -Unique | Where-Object { $_ -and $_.Trim() -ne "" -and $_.Trim() -ne "-" }) -join ", "
                        
                        $dataGridView.Rows.Add("Postes uniques", $uniqueWS)
                        $dataGridView.Rows.Add("IPs uniques", $uniqueIPs)

                        # Affichage du dernier événement
                        $last = $results[0]
                        if ($last.PSObject.Properties.Match('WorkstationName') -and $last.PSObject.Properties.Match('IPAddress')) {
                            $dataGridView.Rows.Add("Dernier Poste", $last.WorkstationName)
                            $dataGridView.Rows.Add("Dernière IP", $last.IPAddress)
                            $dataGridView.Rows.Add("Date Connexion", $last.TimeCreated)
                        }
                        else {
                            $dataGridView.Rows.Add("Dernier Poste", "N/A")
                            $dataGridView.Rows.Add("Dernière IP", "N/A")
                        }
                    }
                    else {
                        Write-LogToBox "Aucun événement trouvé."
                        $dataGridView.Rows.Add("Résultat", "Aucune donnée")
                    }
                }
                else {
                    Write-LogToBox "Type de résultat inconnu : $($results.GetType())"
                }
            ...
            Write-LogToBox "Erreur récupération résultat : $_"
        }
    }
})
...
function StartUserLogSearch {
    ...
    if ($global:Runspace -and $global:Runspace.RunspaceStateInfo.State -eq 'Opened') {
        $global:Runspace.Close()
    }
    ...
    Write-LogToBox "Recherche sur $SelectedDC pour $Username sur $NbDays jours..."

    $eventID = 4624
    $startDate = (Get-Date).AddDays(-$NbDays)

    $scriptBlock = {
        param($DC, $EventID, $StartDate, $UserName)

        try {
            $query = "*[System[(EventID=$EventID) and TimeCreated[@SystemTime >= '$($StartDate.ToUniversalTime().ToString("o"))']] and EventData[Data[@Name='TargetUserName'] = '$UserName']]"
            $results = Get-WinEvent -LogName Security -FilterXPath $query -ComputerName $DC -ErrorAction Stop

            $output = @()
            foreach ($evt in $results) {
                $xml = [xml]$evt.ToXml()
                $ws = $xml.SelectSingleNode("//*[@Name='WorkstationName']").'#text'
                $ip = $xml.SelectSingleNode("//*[@Name='IpAddress']").'#text'
                if ($ws -or $ip) {
            ...
            return $output | Sort-Object -Property TimeCreated -Descending
        } catch {
    ...
    $ps.Runspace = $rs
    $ps.AddScript($scriptBlock).AddArgument($SelectedDC).AddArgument($eventID).AddArgument($startDate).AddArgument($Username)
    $async = $ps.BeginInvoke()

    $global:Runspace = $rs
    $global:PowerShellAsync = $ps
    $global:AsyncResult = $async
...
$buttonSearch.Add_Click({
    ...
    try {
        ...
        # Séparateur
        $dataGridView.Rows.Add("----- Connexions -----", "")

        # Lancer la recherche logs
        StartUserLogSearch -Domain $domain -Username $user -NbDays $nb -SelectedDC $dc
    } catch {
